...
Mọi label và option loại hình đọc từ module dùng chung `src/lib/rental-types.ts`. Hai trục độc lập, đều gắn với **Property** (cụm quản lý), không gắn với tài khoản.
...
Super Admin cấp allowlist qua `enabledRentalTypes` trên tài khoản chủ trọ. Hạn mức phòng thương lượng (`subscribedStandardRoomLimit` / `subscribedColivingRoomLimit`) hiển thị ở tab Gói Roomio và trang admin.
...
Chọn khi tạo/sửa property ở trang Tòa nhà. Badge mô hình vận hành chỉ hiện khi đã phân loại (không hiện `UNSPECIFIED`). Trục B không ảnh hưởng báo giá subscription.
