<script lang="ts">
	...
	> = {
		...
		DORM: {
			...
			workflows: [
				'Check-in/out theo người',
				'Quản lý giường hoặc box trong cùng phòng',
				'Giới tính / khu yên tĩnh',
				'Gói dịch vụ trọn gói'
			],
			billing: [
				'Tiền giường / box',
				'Dịch vụ trọn gói hoặc chia theo đầu người',
	...
	async function loadWorkspace() {
		...
		try {
			...
			if (!propertiesRes.ok)
				throw new Error(propertiesData.error || 'Không tải được danh sách tòa nhà');
	...
	{:else if property}
		<section class="flex flex-col gap-4 lg:flex-row lg:items-start lg:justify-between">
			<div class="min-w-0">
				...
				<div class="mt-3 flex flex-wrap items-center gap-2">
					<h1 class="text-2xl leading-tight font-black text-black">{property.name}</h1>
				...
				<p class="mt-1 text-sm font-bold text-zinc-500">
					{property.shortName} · {property.address}
				</p>
			</div>
			...
			<div class="flex flex-wrap gap-2">
				...
				<a
					href="/dashboard/meters?propertyId={property.id}"
					class="roomio-button-white px-3 py-2 text-xs"
				>
					<Gauge class="h-4 w-4" /> Chốt số
				</a>
				<a
					href="/dashboard/invoices/bulk?propertyId={property.id}"
					class="roomio-button-white px-3 py-2 text-xs"
				>
		...
		<section class="grid gap-5 lg:grid-cols-[minmax(0,1fr)_360px]">
			<div class="space-y-5">
				{#if meta().note}
					<div
						class="rounded-lg border-2 border-black bg-blue-50 px-4 py-3 text-sm font-bold text-blue-900"
					>
						{meta().note}
					</div>
				{/if}
				...
				<div>
					...
					<div class="overflow-hidden rounded-lg border-2 border-black">
						{#each meta().workflows as workflow, index}
							...
							>
								<span
									class="grid h-7 w-7 shrink-0 place-items-center rounded-full bg-zinc-100 text-xs font-black"
									>{index + 1}</span
								>
								<span>{workflow}</span>
							</div>
				...
				<div>
					...
					{#if rooms.length === 0}
						<div
							class="rounded-lg border-2 border-black bg-white px-4 py-8 text-center text-sm font-bold text-zinc-500"
						>
					...
					{:else}
						<div class="overflow-x-auto rounded-lg border-2 border-black">
							<div
								class="grid min-w-[680px] grid-cols-[120px_minmax(180px,1fr)_120px_140px_120px] bg-zinc-50 px-4 py-3 text-xs font-black text-zinc-500"
							>
								<span>Đơn vị</span>
							...
							{#each rooms as room}
								...
								>
									...
									>
									<span
										class="w-fit rounded-full px-2.5 py-1 text-[10px] font-black {statusClass(
											room.status
										)}"
									>
				...
				<div class="rounded-lg border-2 border-black bg-white p-4">
					...
					<div class="mt-3 grid gap-2">
						<a
							...
							class="roomio-button-white justify-between px-3 py-2 text-xs"
						>
							<span class="inline-flex items-center gap-1.5"
								><Users class="h-4 w-4" /> Khách thuê</span
							>
						...
						>
							<span class="inline-flex items-center gap-1.5"
								><Receipt class="h-4 w-4" /> Hợp đồng</span
							>
							<ArrowRight class="h-4 w-4" />
						</a>
					</div>
